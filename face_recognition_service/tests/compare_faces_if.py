                ...
                for uuid_dir in os.listdir(caminho_id):
                    ...
                    if os.path.isdir(caminho_uuid):
                        ...
                        if os.path.exists(caminho_imagem_comparar):
                            ...
                            img1 = cv2.imread(caminho_imagem_raiz)
                            img2 = cv2.imread(caminho_imagem_comparar)
                            faces1 = app_face.get(img1)
                            faces2 = app_face.get(img2)
                            if len(faces1) == 0 or len(faces2) == 0:
                                correspondencia = False
                            else:
                                correspondencia = np.dot(faces1[0].normed_embedding, faces2[0].normed_embedding.T)
